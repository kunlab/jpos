...
public class JCESecurityModule extends BaseSMAdapter{
     ...
     * @param kek the key-encrypting key(明文)
     * @param checkParity if true, the key is not imported unless it has adjusted parity
     * @return imported key secured by the security module
     * @throws SMException if the parity of the imported key is not adjusted AND checkParity = true
     */
    public SecureDESKey  importKeyImpl(short keyLength, String keyType, byte[] encryptedKey,
                                      byte[] kek, boolean checkParity) throws SMException {
        // decrypt encrypted key
        Key clearKEY = jceHandler.decryptDESKey(keyLength,encryptedKey,
                jceHandler.formDESKey(keyLength, kek), checkParity);
     ...
     * @param kekLength the bit length of kek
     * @return the exported key (key encrypted under kek)
     * @throws SMException
     */
    public byte[] exportKeyImpl (SecureDESKey key, byte[] kek, short kekLength) throws SMException {
        // get key in clear
        Key clearKey = decryptFromLMK(key);
        // Encrypt key under kek
        return jceHandler.encryptDESKey(key.getKeyLength(), clearKey, jceHandler.formDESKey(kekLength, kek));
    }
    ...
    byte[] calculateKeyCheckValue (Key key) throws SMException {
        ...
        return ISOUtil.trim(encryptedZeroBlock, 4);
    }
